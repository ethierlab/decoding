...
function W_final = train_decoder(inputs,outputs,params)
tstart = tic;
...
W_final = nan(size(W_init)+[1,0]); %add a line for offset

% gradient descent to optimize and minimize weights
for out = 1:num_out
    fprintf('training decoder for output %d of %d\n',out,num_out);
    tic;
    [W_final(2:end,out),fmin_val,exit_flag,fmin_output,final_grad] = fminunc(@(W) min_weights(inputs,outputs(:,out),W,params.lambda),W_init(:,out),fmin_options);
